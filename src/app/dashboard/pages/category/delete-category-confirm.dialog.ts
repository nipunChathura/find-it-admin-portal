...
export interface DeleteCategoryConfirmData {
  categoryName: string;
}
...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content>
      <p>Are you sure you want to delete <strong>{{ data.categoryName }}</strong>?</p>
      <p class="delete-category-confirm__hint">This action cannot be undone.</p>
    </mat-dialog-content>
  ...
  styles: [
    `
      ...
      .delete-category-confirm__hint { margin-top: 0.5rem; font-size: 0.875rem; color: var(--mat-sys-on-surface-variant); }
...
export class DeleteCategoryConfirmDialogComponent {
  readonly data: DeleteCategoryConfirmData = inject(MAT_DIALOG_DATA);
  private readonly dialogRef = inject(MatDialogRef<DeleteCategoryConfirmDialogComponent>);
