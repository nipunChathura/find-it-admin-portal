...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>Reject Merchant</h2>
    <mat-dialog-content>
      <p class="reject-reason-dialog__intro">Rejecting <strong>{{ data.merchantName }}</strong>. Please enter the reason.</p>
      <mat-form-field appearance="outline" class="reject-reason-dialog__field">
  ...
  styles: [
    `
      ...
      .reject-reason-dialog__intro { margin: 0 0 1rem 0; font-size: 0.9375rem; color: var(--mat-sys-on-surface-variant); }
      .reject-reason-dialog__field { width: 100%; display: block; }
...
export class RejectReasonDialogComponent {
  readonly data: RejectReasonDialogData = inject(MAT_DIALOG_DATA);
